...
# PRIORIDAD GENERAL
SELECT PLANO, DEPENDENCIA, IMPEDIMENTO, DATE_FORMAT(REVISION, '%Y-%m-%d') AS REVISION, TIPO, SUM(`06/12/2021` + `05/12/2021`) AS DAYS, PROBLEMA, ESTADO FROM STATUS_NODE
INNER JOIN NODES ON NODES.ID = STATUS_NODE.ID_NODE
INNER JOIN AFECTED_DAYS ON NODES.ID = AFECTED_DAYS.ID_NODE
WHERE STATUS_NODE.ID_NODE IN (
SELECT ID_NODE FROM AFECTED_DAYS
WHERE `06/12/2021` + `05/12/2021` >= 2)
...
SELECT CMTS, PLANO, DEPENDENCIA, IMPEDIMENTO, DATE_FORMAT(REVISION, '%Y-%m-%d') AS REVISION, TIPO, SUM(`18/12/2021` + `17/12/2021` + `16/12/2021` + `14/12/2021` + `13/12/2021` ) AS DAYS, PROBLEMA, ESTADO, DETALLE FROM STATUS_NODE
    INNER JOIN NODES ON NODES.ID = STATUS_NODE.ID_NODE
    INNER JOIN AFECTED_DAYS ON NODES.ID = AFECTED_DAYS.ID_NODE
